...
def format_answer(parsed_answer: dict, output="full") -> None:
    ...
    if output == "full":
        ...
        time = parsed_answer["time"]
        if time <= 3000:
            click.echo(f"\n{click.style('Time:', bold=True)} {click.style(str(time), fg='green')} ms\n")
        elif 3000 < time < 8000:
            click.echo(f"\n{click.style('Time:', bold=True)} {click.style(str(time), fg='yellow')} ms\n")
        else:
            click.echo(f"\n{click.style('Time:', bold=True)} {click.style(str(time), fg='red')} ms\n")
